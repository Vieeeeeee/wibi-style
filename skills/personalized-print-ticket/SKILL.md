...
description: 把一张人物、风景、建筑、空间、食物或日常随拍制作成中英混排的定制印刷票根。先根据照片内容动态询问，再从四套票根 Prompt 中选一套；代码产出准确文字和构图的排版导引图，然后将导引图、用户照片和印刷风格参考共同交给图像模型，生成完整票根，最后自动加入四角裁口、照片分界撕票口、虚线、打孔和黑底展示。用于定制票根、电影票风格照片卡、旅行纪念票或日常瞬间票据。
---
...
9. 成品通过检查后运行裁切脚本。脚本使用票根真实照片分栏比例 `1472/2048`，生成四角内凹、分界撕票口、虚线和侧边打孔的黑底展示图，同时保留透明裁切版：

```bash
python3 {baseDir}/scripts/finish_ticket.py \
  --ticket 最终票根.png \
  --out 票根裁切展示.png \
  --transparent-out 票根透明裁切.png
```
...
先展示黑底票根裁切图，再用一到两句说明选用的风格、图像方式和保留的身份锚点。同时交付黑底展示 PNG、透明裁切 PNG、原始横向票根 PNG、信息 JSON 和本次完整 Prompt；排版导引图作为过程资产保留。
