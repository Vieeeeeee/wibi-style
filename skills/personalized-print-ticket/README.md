...
6. 生成通过检查后，用代码沿照片副券边缘加入撕票虚线与缺口，并输出带四角裁口、侧边打孔且票面外完全透明的独立 PNG。
...
默认先生成 `2048×769` 的横向窄票根，再输出带真实透明通道的独立裁切票根。裁切严格跟随照片副券边缘，包含四角裁口、上下撕票口、虚线和侧边打孔；票面以外没有白底或黑底。Skill 同时保留原始横向票根、信息 JSON、完整生成 Prompt 和排版导引图；一次调用只生成一张，失败或视觉未通过时不会自动重试。
