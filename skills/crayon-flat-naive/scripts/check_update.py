...
def main() -> int:
    ...
    url = os.environ.get("WIBI_STYLE_UPDATE_MANIFEST", local["update_manifest_url"])
    parsed = urllib.parse.urlsplit(url)
    checked = f"{url}{'&' if parsed.query else '?'}checked_at={int(time.time())}" if parsed.scheme in {"http", "https"} else url
    try:
