...
def main() -> int:
    ...
    checked_url = f"{remote_url}{separator}checked_at={int(time.time())}" if parsed.scheme in {"http", "https"} else remote_url
    request = urllib.request.Request(checked_url, headers={"User-Agent": "wibi-style-update-check/1", "Cache-Control": "no-cache"})
    try:
        with urllib.request.urlopen(request, timeout=args.timeout) as response:
            remote = json.loads(response.read().decode("utf-8"))
        if remote.get("slug") != local.get("slug"):
            raise ValueError("remote manifest slug does not match local skill")
        current, latest = local["version"], remote["version"]
        status = "UPDATE_AVAILABLE" if version_key(latest) > version_key(current) else "UP_TO_DATE"
        result = {"status": status, "slug": local["slug"], "current": current, "latest": latest, "install_url": remote.get("install_url", local["install_url"])}
    ...
    if args.as_json:
        ...
        print(f"UPDATE_AVAILABLE current={result['current']} latest={result['latest']} install_url={result['install_url']}")
