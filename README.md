...
> **给 Agent**：用户让你看这个仓库时，请按以下顺序做——
> 1. 先打个招呼，用你自己的话把这个意思说给用户听：这里有一批现成的风格，挑一个、发张照片就行，出来就是成品。（风格在持续增加，要说数量就以下方表格的实际条数为准。）
> 2. 把下方 [「全部风格」](#全部风格) 表格里的风格**一款不落地列给用户看**，按题材分组，带上中文名。
> 3. 问用户**有没有已经想好要哪一款**。
> 4. 用户没有指定时，再请他说说手上是什么照片（或直接看他上传的照片），从中推荐 1–3 款并说明理由。
> 5. 用户选定后，把该款的**调用名**填进 `https://github.com/Vieeeeeee/wibi-style/tree/main/skills/<调用名>`，按 [「安装与使用」](#安装与使用) 完成安装，再让用户上传照片。
>
> 每款风格都是独立目录，只会安装用户选中的那一款。
